...
import { buildFormData, buildProductFormData } from "../Utils/helper";
...
export const getAllNotification = async (route, params = {}) => {
    try {
        const { data } = await axios.get(route, { params });
        return data;
    } catch (error) {
...
export const productPost = async (route, params = {}) => {
    try {
        const formData = new FormData();
        buildProductFormData(formData, params);
        const { data } = await axios.post(route, formData);
        return data;
    } catch (error) {
        console.error('Error posting data:', error);
