...
const mapProduct = (p) => {
  ...
  const variants = skus.map((sku) => {
      ...
      return { name: vname, image: vimage, hex: hexFromColorName(vname), price: vprice, vstock, sku_id: sku.sku_id, stock: sku.stock }
  })
  const priceNum = (() => {
      const nums = variants.map((v) => parseFloat(v.price ?? '0')).filter((n) => !Number.isNaN(n) && n > 0)
      if (nums.length) return Math.min(...nums)
      const p0 = parseFloat(p.price ?? '0'); return Number.isNaN(p0) ? 0 : p0
  })()
  const currency = p.currency || (skus[0]?.currency) || 'USD'
  const priceStr = priceNum > 0 ? `${currency} ${priceNum}` : `${currency} ${p.price || '0.00'}`
  const image = variants[0]?.image || imageFallback
  return { id: p.id || p.ae_product_id || p.slug, name, description, price: priceStr, image, variants, currency, stock: typeof p.stock === 'number' ? p.stock : null }
}
...
const ProductView = () => {
    ...
    const handleAddToCart = () => {
        // Validate required selections
        if (productData?.variants && productData.variants.length > 0 && !selectedColor) {
            showToast("Please select a color", "warning");
            return;
        }
                        ...
                        <Row>
                            <Col md={6}>
                                <div className="product-media-wrap">
                                    ...
                                    <div className="product-media-main">
                                        <img 
                                            src={getCurrentImages()[currentImageIndex] || productData?.image} 
                                            alt={productData?.name || "Product"} 
                                            className="main-product-image"
                                        />
                                    </div>
                                    ...
                                    <div className="product-view__info">
                                        ...
                                        <div className="product-view__price mb-4">
                                            <div className="price fw-semibold">
                                                <span className='currency-code'>{productData?.currency || 'CHF'}</span>
                                        ...
                                        {productData?.variants && productData.variants.length > 0 && (
                                            <div className="mb-3">
                                                <strong>Colors:</strong> <span>{selectedColor || 'Select a color'}</span>
                                                <div className="d-flex gap-2 mt-2">
                                                    {productData.variants.map((variant) => (
                                                        <div
                                                            key={variant.name}
                                                            className={`color-swatch ${selectedColor === variant.name ? "selected" : ""}`}
                                                            style={{ backgroundColor: variant.hex || hexFromColorName(variant.name) }}
