...
const ProductView = () => {
  ...
  //     if (window.scrollY > left.offsetTop - topOffset &&
  ...
  //       });
  //     } else if (window.scrollY >= left.offsetTop + left.offsetHeight - rightHeight - topOffset) {
  //       // stop at bottom
  //       setStickyStyle({
  //         position: "absolute",
  //         bottom: "0",
  ...
  // Handle scroll behavior for image slider
  const handleImageScroll = (e) => {
    ...
    // Check if we're at the bottom of the image slider
    const isAtBottom = scrollTop + clientHeight >= scrollHeight - 5;

    if (isAtBottom && isScrollingImages) {
      // Release scroll lock when reaching bottom
      setIsScrollingImages(false);
      document.body.style.overflow = "auto";
    }
  };

  // Handle wheel events on image slider
  const handleWheelOnSlider = (e) => {
    if (isScrollingImages) {
      e.preventDefault();
      const slider = imageSliderRef;
      if (slider) {
  ...
  // Handle wheel events on main image area
  const handleWheelOnMainImage = (e) => {
    if (!isScrollingImages) {
      // Start scrolling images when wheel is used on main image
      setIsScrollingImages(true);
