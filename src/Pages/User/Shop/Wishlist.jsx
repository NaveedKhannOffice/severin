...
const Wishlist = ({ filters, setFilters, pagination, updatePagination }) => {
    ...
    const fetchBookings = async () => {
        try {
            ...
            const response = await getAll(`/user/wishlist?type=App\\Models\\Service`, filters);
            if (response) {
    ...
    const handleToggleWishList = async (id) => {
        ...
        try {
            ...
            let response = await post(`/user/wishlist/add`, { wishlistable_id: id, wishlistable_type: `App\\Models\\Service` });
            fetchBookings();
                        ...
                        {wishList?.map((item, index) => (
                            <Col xs={12} lg={3} xl={4} key={index} className='mb-3 mb-xl-4 mb-xxl-5'>
                                <GeneralCard
                                    ...
                                    linkPath="/services"
                                    toggleWishList={() => handleToggleWishList(item?.wishlistable?.id)}
                                />
                            </Col>
                        ))}
                    </Row>
