...
import { addBlogSchema, editBlogSchema } from "../../../../Config/Validations";
...
const EditBlog = ({ showModal }) => {
  ...
  const handleSubmit = async (values) => {
    ...
    showModal(`Update Blog`, `Are you sure you want to update this Blog?`, () =>
  ...
  const onConfirmStatusChange = async (values) => {
    ...
    const response = await post(`/admin/blogs/${id}`, values);
    if (response.status) {
      showModal(
        "Successful",
        `This Blog has been updated successfully!`,
            ...
            <div className="col-12">
              <Formik
                initialValues={{
                  ...
                  // banner: "",
                  // file: "",
                  banner: blog?.banner?.media_path
                    ? [blog.banner.media_path]
                    : [],
                  file: blog?.file?.media_path ? [blog.file.media_path] : [],
                }}
                      ...
                      <div className="col-md-8 col-lg-6 col-xl-5 col-xxl-4 mb-3">
                        <div className="radio-checkbox-wrapper gap-3 gap-sm-5">
                          <label className="radio-checkbox-container">
                            <input
                              defaultChecked={blog.category === "free"}
                              onChange={handleChange}
                              type="radio"
                              name="category"
                              value="free"
                          ...
                          <label className="radio-checkbox-container">
                            <input
                              defaultChecked={blog.category === "premium"}
                              onChange={handleChange}
                              type="radio"
                              name="category"
                              value="premium"
