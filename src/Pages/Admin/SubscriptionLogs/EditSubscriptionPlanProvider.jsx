...
import { getDetails, post, put } from "../../../Services/Api";
...
const EditSubscriptionPlanProvider = ({ showModal }) => {
  ...
  {
    console.log(subscriptionPlan, "ini valuesss");
  }

  const getPlan = async () => {
    const response = await getDetails(`/admin/plans/${id}`);
    if (response.status) {
  ...
  const handleSubmit = async (values, { resetForm }) => {
    console.log(values);
    startSubmitting();
    const response = await post(`/admin/plans/${id}`, values);
